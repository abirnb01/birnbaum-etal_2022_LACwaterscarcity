...
for (bs in basin_list) {
  ...
  no_dups$profit_quantile <- no_dups$profit_quantile #make sure that they're all integers
  sum(no_dups$profit_quantile) #135/2696
  
  df_class <- subset(no_dups, select = c("soc","ag","ssp","tax","gw","esm","res","profit_quantile"))
  df_regr <- subset(no_dups, select = c("soc","ag","ssp","tax","gw","esm","res","profit_pchange"))
  
  #cart using rpart --> regression
  fit_regr <- rpart(profit_pchange ~ .,
                    method="anova", data=df_regr,control=c(maxdepth=4),cp=.000001)
  png(file=paste0("/cluster/tufts/lamontagnelab/abirnb01/GCAM_queries/query_results/CART_trees/ag_profit/",bs,"_",yr,"_regr.png"))
  rpart.plot(fit_regr)
  dev.off()
  
  #now try classification
  #if price_scenario>glob_avg
  fit_class <- rpart(profit_quantile ~ .,
                     method="class", data=df_class)
                     
                     #,control=c(maxdepth=4),cp=.000001)
  png(file=paste0("/cluster/tufts/lamontagnelab/abirnb01/GCAM_queries/query_results/CART_trees/ag_profit/",bs,"_",yr,"_class.png"))
