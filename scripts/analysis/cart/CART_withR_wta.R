...
for (bs in basin_list) {
  ...
  #get most severe scenarios (WTA > 0.4)
  no_dups$scarcity_binary <- no_dups$scarcity > 0.4
  
  
  df_regr <- subset(no_dups, select = c("soc","ag","ssp","tax","gw","esm","res","scarcity"))
  df_class <- subset(no_dups, select = c("soc","ag","ssp","tax","gw","esm","res","scarcity_binary"))
  
  #cart using rpart --> regression
  fit_regr <- rpart(scarcity ~ .,
                    method="anova", data=df_regr,control=c(maxdepth=4),cp=.000001)
  png(file=paste0("/cluster/tufts/lamontagnelab/abirnb01/GCAM_queries/query_results/CART_trees/wta/",bs,"_",yr,"_regr.png"))
  rpart.plot(fit_regr)
  dev.off()
  
  #now try classification
  #if price_scenario>glob_avg
  fit_class <- rpart(scarcity_binary ~ .,
                     method="class", data=df_class,control=c(maxdepth=4),cp=.000001)
  png(file=paste0("/cluster/tufts/lamontagnelab/abirnb01/GCAM_queries/query_results/CART_trees/wta/",bs,"_",yr,"_class.png"))
